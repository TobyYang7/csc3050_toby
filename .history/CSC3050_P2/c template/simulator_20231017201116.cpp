...
{ // split an MIPS instruction, ignore the trailing comment(if exists)
...
{ // get the first word of string s
	string result;
...
void Simulator::syscall(ifstream &ioIn, ofstream &ioOut)
{
	...
	{ // print_string
		...
		int i = 0;
		while (this->memory_pointer[addr + i - BASE_ADDR] != '\0')
		{ // read the null-terminated string, beginning at addr
...
void Simulator::storeAsm(ifstream &asmFile)
{
	...
	while (getline(asmFile, s))
	{
		eraseWhitespaces(s);
		if (!s.empty())
		{ // ignore blank line with leading whitespace
			if (s[0] != '#' && s[0] != '\n')
			{
...
void Simulator::storeBin(ifstream &binFile)
{ // store machine code input
	string s;
...
int Simulator::fetch(uint32_t pc)
{ // fetch the 4 bytes starting at pc and convert it to binary string
